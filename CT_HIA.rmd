...
sum(weights_post$prop_units[weights_post$moved_from == "Waterbury"]) #should sum to 1
sum(weights_post$lambda[weights_post$moved_from == "Branford"]) #should sum to 1

weights_post$total_at_or_below_ami[weights_post$moved_from == "New Haven"][1] #pre-move pop in given town
sum(weights_post$pop_moved[weights_post$moved_from == "New Haven"]) #total families moved out of given town, should be no more than pre-move pop

sum(weights_post$pop_moved) #total families drawn to move or stay, should be no more than total families across all towns
sum(weights_post$total_at_or_below_ami[weights_post$moved_to == "Canaan"]) #should equal total families across all towns
