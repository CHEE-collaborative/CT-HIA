...
# read statewide all-cause mortality rates from CDC WONDER (non-Hispanic White, non-Hispanic Black)
mortality_black_white <- read.delim("allcause_2019_race_ethnicity.txt", sep = "\t")[c(8,14), c(2,4,8)]

mortality_asian <- read.delim("allcause_2019_race.txt", sep = "\t")[2, c(2,6)]

mortality_hispanic <- read.delim("allcause_2019_ethnicity.txt", sep = "\t")[1, c(2,6)]
...
mortality <- bind_rows(mortality_black_white, mortality_asian, mortality_hispanic) %>% 
  ...
  dplyr::select(race_ethnicity, mr_pre = Crude.Rate) %>% 
...
pm_town <- tract_town %>%
  ...
  group_by(`TOWN_NAME`) %>% 
  summarize(pm_mean = mean(pm_mean, na.rm = TRUE), `Standard Deviation (ug/m3)` = sd(pm_mean, na.rm = TRUE), `Min` = min(pm_mean, na.rm = TRUE), `Max` = max(pm_mean, na.rm = TRUE), `IQR (ug/m3)` = IQR(pm_mean, na.rm = TRUE))

# summarize mean and standard deviation of tract-level ozone estimates by town
o3_town <- tract_town %>%
  right_join(o3_tract, by = "GEOID") %>%
  group_by(`TOWN_NAME`) %>% 
  summarize(o3_mean = mean(o3_mean, na.rm = TRUE), `Standard Deviation (ppb)` = sd(o3_mean, na.rm = TRUE), `Min O3` = min(o3_mean, na.rm = TRUE), `Max O3` = max(o3_mean, na.rm = TRUE), `IQR (ppb)` = IQR(o3_mean, na.rm = TRUE)) 
...
ggplot() + geom_sf(data = ct_tracts, aes(fill = pm_mean)) + 
  scale_fill_viridis_c(option = "magma", direction = -1) + geom_sf(data = town_lines)

# map O3 by census tract with town lines
ggplot() + geom_sf(data = ct_tracts, aes(fill = o3_mean)) + scale_fill_viridis_c(option = "magma", direction = -1) + geom_sf(data = town_lines)
```
...
```{r define exposures and dose-response relationships}
...
averted_by_race <- post_sim %>%
  ...
  summarize(across(c(hif_pm, cl_pm, cu_pm, hif_o3, cl_o3, cu_o3, hif_no2, cl_no2, cu_no2, hif_heat, cl_heat, cu_heat), ~ -sum(.x)))

head(post_sim)
head(post_heat)
averted_by_race

# simulate_pop_moved <- function() {
#   simulate_all()
#   return(towns_moved)
# }
# 
# simulate_averted_by_race <- function() {
#   simulate_all()
#   return(averted_by_race)
# }

```

```{r pollutant exposure pre/post summary table}

# pollutants_prepost <- post_sim %>% 
#   dplyr::select(moved_from, moved_to, race_ethnicity, pop_moved, pm_pre, pm_post, o3_pre, o3_post, no2_pre, no2_post)

heat_annual <- heat_town_sf %>% 
  group_by(NAME) %>% 
  summarize(heat_mean = mean(heatindex))
  
heat_annual <- heat_annual %>% rename(TOWN_NAME = NAME)

pollutants_prepost <- pop_town_moved %>% 
  left_join(pm_town[,1:2], by = "TOWN_NAME") %>% 
  left_join(o3_town[,1:2], by = "TOWN_NAME") %>% 
  left_join(no2_town[,1:2], by = "TOWN_NAME") %>% 
  left_join(heat_annual, by = "TOWN_NAME") %>% 
  mutate(across(c(pm_mean, o3_mean, no2_mean, heat_mean), ~.x*pop_pre, .names = "{.col}_pre")) %>% 
  mutate(across(c(pm_mean, o3_mean, no2_mean, heat_mean), ~.x*pop_post, .names = "{.col}_post")) %>% 
  ungroup() %>% 
  summarize(pop_pre = sum(pop_pre), pop_post = sum(pop_post), pm_pre = sum(pm_mean_pre), pm_post = sum(pm_mean_post), o3_pre = sum(o3_mean_pre), o3_post = sum(o3_mean_post), no2_pre = sum(no2_mean_pre), no2_post = sum(no2_mean_post), heat_pre = sum(heat_mean_pre), heat_post = sum(heat_mean_post)) %>% 
  mutate(across(1:10, ~ .x/pop_pre))

head(pollutants_prepost)
...
```{r run simulation x number of times}

# pop_moved_rep <- replicate(3, simulate_pop_moved())
# 
# averted_by_race_rep <- replicate(100, simulate_averted_by_race())
...
ggplot() + geom_sf(data = total_sim, aes(fill = all_pre)) + 
  scale_fill_viridis_c(option = "magma", direction = -1, limits = c(0, 40000)) + theme_minimal() 
# map families by town post-move
ggplot() + geom_sf(data = total_sim, aes(fill = all_post)) + 
  scale_fill_viridis_c(option = "magma", direction = -1, begin = 0, end = 1, limits = c(0, 40000)) + theme_minimal()
...
# bar chart, total deaths averted by pollutant/exposure
averted_totals <- averted_by_race %>% 
  ungroup() %>% 
  summarize(across(hif_pm:cu_heat, ~ sum(.x))) 

averted_totals_2 <- averted_totals %>% 
  mutate(across(hif_pm:cu_heat, ~ round(.x, 0))) %>% 
  mutate(PM2.5 = paste0(hif_pm, " (", cu_pm, ", ", cl_pm, ")")) %>% 
  mutate(O3 = paste0(hif_o3, " (", cu_o3, ", ", cl_o3, ")")) %>% 
  mutate(NO2 = paste0(hif_no2, " (", cl_no2, ", ", cu_no2, ")")) %>%
  mutate(Heat = paste0(hif_heat, " (", cl_heat, ", ", cu_heat, ")")) %>% 
  dplyr::select(PM2.5:Heat)

# format with 95% CI's
averted_by_race_tbl <- averted_by_race %>% 
  mutate(across(hif_pm:cu_heat, ~ round(.x, 0))) %>% 
  mutate(PM2.5 = paste0(hif_pm, " (", cu_pm, ", ", cl_pm, ")")) %>% 
  mutate(O3 = paste0(hif_o3, " (", cu_o3, ", ", cl_o3, ")")) %>% 
  mutate(NO2 = paste0(hif_no2, " (", cl_no2, ", ", cu_no2, ")")) %>%
  mutate(Heat = paste0(hif_heat, " (", cl_heat, ", ", cu_heat, ")")) %>% 
  dplyr::select(race_ethnicity, PM2.5:Heat)




averted_bar <- averted_totals %>% 
  pivot_longer(cols = c(hif_pm, hif_o3, hif_no2, hif_heat), names_to = "Pollutant", values_to = "Deaths Averted") %>%
  mutate(cl = case_match(Pollutant, "hif_pm" ~ cl_pm, "hif_o3" ~ cl_o3, "hif_no2" ~ cl_no2, "hif_heat" ~ cl_heat)) %>% 
  mutate(cu = case_match(Pollutant, "hif_pm" ~ cu_pm, "hif_o3" ~ cu_o3, "hif_no2" ~ cu_no2, "hif_heat" ~ cu_heat)) %>% 
  mutate(Pollutant = case_match(Pollutant, "hif_pm" ~ "PM2.5", "hif_o3" ~ "O3", "hif_no2" ~ "NO2", "hif_heat" ~ "Heat")) %>% 
  dplyr::select(Pollutant:cu)

averted_barchart <- ggplot() + geom_col(data = averted_bar, aes(x = Pollutant, y = `Deaths Averted`, fill = Pollutant)) + geom_errorbar(data = averted_bar, aes(x = Pollutant, ymin = cl, ymax = cu), width = 0.2) + theme_minimal()

ggsave("averted_bar_chart.png", averted_barchart, dpi = 700, width = 5, height = 4)

# rates

pop_race <- pop_town_moved %>% 
  group_by(race_ethnicity) %>% 
  summarize(pop_pre = sum(pop_pre)) %>% 
  left_join(mortality, by = "race_ethnicity") %>% 
  mutate(deaths_pre = mr_pre*pop_pre*4/100000)

averted_by_race_rates <- averted_by_race %>% 
  left_join(pop_race, by = "race_ethnicity") %>% 
  mutate(across(2:13, ~ round(.x/deaths_pre*100, 2))) %>%
  mutate(PM2.5 = paste0(hif_pm, "% (", cu_pm, ", ", cl_pm, ")")) %>% 
  mutate(O3 = paste0(hif_o3, "% (", cu_o3, ", ", cl_o3, ")")) %>% 
  mutate(NO2 = paste0(hif_no2, "% (", cl_no2, ", ", cu_no2, ")")) %>%
  mutate(Heat = paste0(hif_heat, "% (", cl_heat, ", ", cu_heat, ")")) %>% 
...
total_deaths <- sum(pop_race$deaths_pre)

total_rates <- averted_totals %>% 
  mutate(deaths_pre = round(total_deaths, 0)) %>% 
  mutate(across(1:12, ~ round(.x/deaths_pre*100, 2))) %>% 
  mutate(PM2.5 = paste0(hif_pm, "% (", cu_pm, ", ", cl_pm, ")")) %>% 
  mutate(O3 = paste0(hif_o3, "% (", cu_o3, ", ", cl_o3, ")")) %>% 
  mutate(NO2 = paste0(hif_no2, "% (", cl_no2, ", ", cu_no2, ")")) %>%
  mutate(Heat = paste0(hif_heat, "% (", cl_heat, ", ", cu_heat, ")")) %>% 
